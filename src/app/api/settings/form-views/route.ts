...
export async function GET(request: NextRequest) {
  try {
    ...
    const items = await listEntityFormViews(
      prisma,
      session.tenantId,
      entity,
      activeOnly,
    );
    return NextResponse.json({ items: items.map(serializeFormView) });
  } catch (error) {
...
export async function POST(request: Request) {
  try {
    ...
    const body = formViewUpsertSchema.parse(await request.json());
    if (body.isDefault) {
    ...
    const item = await prisma.entityFormView.create({
      data: {
        tenantId: session!.tenantId,
        entity: body.entity,
        code: body.code,
    ...
    await writeAuditEvent({
      tenantId: session!.tenantId,
      userId: session!.sub,
      action: "settings.form_views.create",
      entity: "entity_form_view",
      entityId: item.id,
      meta: { code: item.code, entity: item.entity },
    });
    return NextResponse.json({ item: serializeFormView(item) }, { status: 201 });
  } catch (error) {
    if (error instanceof z.ZodError) {
