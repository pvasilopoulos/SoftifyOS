...
import { ORDER_WORKFLOWS } from "@/modules/sales/order-utils";
import {
  deleteOrderStatusOption,
...
const patchSchema = z.object({
  ...
  workflow: z.enum(ORDER_WORKFLOWS).optional(),
...
export async function PATCH(
  ...
  try {
    ...
    const item = await updateOrderStatusOption(prisma, {
...
export async function DELETE(
  ...
  try {
    ...
    const { id } = await context.params;
    await deleteOrderStatusOption(prisma, {
      tenantId: session.tenantId,
      id,
    });
    await writeAuditEvent({
