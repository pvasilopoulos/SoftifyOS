...
function fromItem(m: OrderStatusItem): Draft {
  return {
    code: m.code,
    name: m.name,
    workflow: m.workflow,
...
export function OrderStatusesClient({
  initialItems,
}: {
  initialItems: OrderStatusItem[];
}) {
  ...
  const openEdit = (m: OrderStatusItem) => {
    setCreating(false);
    setEditingId(m.id);
    setDraft(fromItem(m));
    setError(null);
  ...
  const save = (e: FormEvent) => {
    ...
    startTransition(async () => {
      ...
      const res = creating
        ? await fetch("/api/settings/order-statuses", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(payload),
          })
        : await fetch(`/api/settings/order-statuses/${editingId}`, {
            method: "PATCH",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(payload),
          });
  ...
  const remove = () => {
    ...
    startTransition(async () => {
      const res = await fetch(`/api/settings/order-statuses/${editing.id}`, {
        method: "DELETE",
      });
      const data = await res.json();
      if (!res.ok) {
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <PageHeader
            ...
            description="Παραμετρικές ετικέτες με αντιστοίχιση σε workflow (πρόχειρη, επιβεβαιωμένη…)."
          />
        </div>
            ...
            {items.map((m) => (
              <tr key={m.id} className="border-b border-slate-100 last:border-0">
                ...
                <td className="px-4 py-3 text-slate-600">
                  {orderStatusLabel[m.workflow] ?? m.workflow}
                </td>
          ...
          >
            ...
            <div className="flex-1 space-y-4 overflow-y-auto px-4 py-4">
              ...
              <label className="block text-sm">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setDraft((d) => ({
                      ...d,
                      workflow: e.target.value as OrderStatusKey,
                    }))
                  }
                >
                  {ORDER_WORKFLOWS.map((w) => (
                    <option key={w} value={w}>
                      {orderStatusLabel[w]} ({w})
                    </option>
                  ))}
                </select>
              ...
              <label className="flex items-center gap-2 text-sm">
                ...
                />
                Εμφάνιση στη φόρμα νέας παραγγελίας/προσφοράς
              </label>
