...
import { orderKindLabel } from "@/modules/sales/order-utils";

export type OrderDocKind = "SALES_ORDER" | "SALES_QUOTE";
...
export function NewOrderForm({ kind = "SALES_ORDER" }: { kind?: OrderDocKind }) {
  ...
  async function onSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    router.push(`${listHref}/${data.item!.id}`);
    router.refresh();
  }
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-5">
      ...
      <PageHeader
        title={`Νέα ${title.toLowerCase()}`}
        description={
          isQuote
            ? "Προσφορά πελάτη · χωρίς δέσμευση αποθέματος."
            : "Επιλέξτε πελάτη και προϊόντα από τον κατάλογο."
        }
      />
      ...
      <form onSubmit={onSubmit} className="space-y-5">
        ...
        <div className="flex flex-wrap gap-3">
          <Button type="submit" disabled={pending || !seriesId}>
            {pending ? "Αποθήκευση..." : `Αποθήκευση ${title.toLowerCase()}`}
          </Button>
          <Button
            type="button"
            variant="secondary"
