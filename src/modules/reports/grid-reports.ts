...
export async function runGridApOpen(
  ...
  const rows = ap.map((r) => ({
    ...
    orderedAt: dateLabel(r.orderedAt),
    status: r.status,
    total: r.total,
  }));
  ...
  return gridResult({
    ...
    description: "Ανοιχτές παραγγελίες αγοράς",
    periodLabel: "Τρέχουσες ΠΟ",
    pageSize: 50,
    kpis: [
      { label: "Παραγγελίες", value: rows.length },
      { label: "Αξία", value: money(sumCol(rows, "total")) },
    ],
    columns: [
      { key: "number", label: "ΠΟ", type: "string", sortable: true },
      { key: "supplierCode", label: "Κωδ. προμηθ.", type: "string", sortable: true },
      { key: "supplier", label: "Προμηθευτής", type: "string", sortable: true },
      { key: "orderedAt", label: "Ημ/νία", type: "date", sortable: true },
      { key: "status", label: "Κατάσταση", type: "string", sortable: true },
      { key: "total", label: "Αξία", type: "currency", align: "end", sortable: true },
    ],
    rows,
    totals: {
      number: `${rows.length} ΠΟ`,
      total: sumCol(rows, "total"),
    },
  });
}
